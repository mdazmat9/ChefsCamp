...
class ListProblems extends Component {
    ...
    componentDidMount(){
      ...
      axios.get(`http://localhost:88/contests/${this.props.contestCode}/${userName}`)
         .then(res => {
            this.props.handleLiftContestDetail(res.data);
            if(res.data.result.data.content.problemsList.length > 0){
               this.setState({loaded:true})
            }else{
               this.setState({loaded:true, message:"Problems not currently available . . .", is404: true})
            }
         });
    }
    renderTableData() {
        return this.props.data.result.data.content.problemsList.map((problem, index) => {  
           return (
              <tr key={index}>
       ...
       if(this.state.is404){
         return(
            <div style={{height:'150px', width:'620px', paddingTop:'150px'}}>
               <center><strong>{this.state.message}</strong></center>
            </div>
         );
       }else if(this.state.redirect){
          return(
             <Redirect to={{pathname:`${this.props.contestCode}/problem/${this.state.selected}`}} />
          );
       }else{
